...
public sealed class WiiRuntimeSourceTests {
    ...
    public void DrawFailureDiagnostic_MapsSceneManagerTraceStages() {
        ...
        string applicationHeaderSource = File.ReadAllText(Path.Combine(repositoryRootPath, "src", "platform", "wii", "WiiApplication.hpp"));
        string applicationSource = File.ReadAllText(Path.Combine(repositoryRootPath, "src", "platform", "wii", "WiiApplication.cpp"));
        string normalizedFailureCodeSource = failureCodeSource.ReplaceLineEndings("\n");
        string normalizedApplicationSource = applicationSource.ReplaceLineEndings("\n");

        Assert.Contains("/// Immediate scene loading had entered or was about to resolve the requested scene's content path.\n        ScenePathResolution = 0xC111U", normalizedFailureCodeSource, StringComparison.Ordinal);
        Assert.Contains("/// Pending scene-operation commit was entering, executing a queued operation, or completing at the generated frame boundary.\n        SceneOperationCommit = 0xC110U", normalizedFailureCodeSource, StringComparison.Ordinal);
        Assert.Contains("/// Immediate scene loading had resolved the content path and was looking up the loaded-scene record or performing single-mode pre-load cleanup.\n        /// Cleanup includes disposing untracked roots, unloading existing scenes, flushing released textures, and resetting physics timing.\n        SceneRecordLookup = 0xC112U", normalizedFailureCodeSource, StringComparison.Ordinal);
        Assert.Contains("/// Immediate scene loading was about to load the resolved scene content.\n        SceneContentLoad = 0xC113U", normalizedFailureCodeSource, StringComparison.Ordinal);
        Assert.Contains("/// The scene manager was about to call the scene-load service to materialize resolved scene content.\n        SceneMaterializationCall = 0xC114U", normalizedFailureCodeSource, StringComparison.Ordinal);
        Assert.Contains("/// The scene-load service had returned materialized scene content to the scene manager.\n        SceneMaterializationReturn = 0xC115U", normalizedFailureCodeSource, StringComparison.Ordinal);
        Assert.Contains("/// The scene manager was about to track the newly materialized loaded-scene record.\n        SceneRecordTrack = 0xC116U", normalizedFailureCodeSource, StringComparison.Ordinal);
        Assert.Contains("/// The scene manager had inserted the newly materialized record into its ordered loaded-scene list.\n        SceneRecordListInsertion = 0xC117U", normalizedFailureCodeSource, StringComparison.Ordinal);
        Assert.Contains("/// The scene manager had inserted the newly materialized record into its loaded-scene lookup dictionary.\n        SceneRecordDictionaryInsertion = 0xC118U", normalizedFailureCodeSource, StringComparison.Ordinal);
        Assert.Contains("/// The scene manager was about to dispatch the scene-loaded event for the newly tracked scene.\n        SceneLoadedEvent = 0xC119U", normalizedFailureCodeSource, StringComparison.Ordinal);
        Assert.Contains("/// The scene-loaded event dispatch had returned control to generated core code.\n        SceneLoadedEventReturned = 0xC11AU", normalizedFailureCodeSource, StringComparison.Ordinal);
        Assert.Contains("/// Generated core code was about to release the scene-loaded event arguments after dispatch.\n        SceneLoadedEventArgsRelease = 0xC11BU", normalizedFailureCodeSource, StringComparison.Ordinal);
        Assert.Contains("/// Generated core code had released the scene-loaded event arguments after dispatch.\n        SceneLoadedEventArgsReleased = 0xC11CU", normalizedFailureCodeSource, StringComparison.Ordinal);
        Assert.Contains("/// Immediate scene loading had returned from the scene-loaded event or reached its end, or transition cleanup had released scene assets or committed the transition.\n        SceneLoadCleanup = 0xC11DU", normalizedFailureCodeSource, StringComparison.Ordinal);
        Assert.Contains("/// Scene materialization had begun or was about to load the authored root entity.\n        SceneMaterializationBegin = 0xC120U", normalizedFailureCodeSource, StringComparison.Ordinal);
        Assert.Contains("/// Scene materialization had begun constructing an authored entity.\n        SceneEntityConstruction = 0xC121U", normalizedFailureCodeSource, StringComparison.Ordinal);
        Assert.Contains("/// Scene materialization was preparing to deserialize or had begun deserializing an authored component with no more specific mapping.\n        SceneComponentDeserialization = 0xC122U", normalizedFailureCodeSource, StringComparison.Ordinal);
        Assert.Contains("/// Scene materialization was about to load an authored child entity.\n        SceneChildEntity = 0xC123U", normalizedFailureCodeSource, StringComparison.Ordinal);
        Assert.Contains("/// Scene materialization had completed loading an authored entity or was initializing the materialized hierarchy after the final entity completed.\n        SceneEntityCompletion = 0xC124U", normalizedFailureCodeSource, StringComparison.Ordinal);
        Assert.Contains("/// Scene materialization had completed reconstruction of the entire authored scene graph.\n        SceneMaterializationCompleted = 0xC125U", normalizedFailureCodeSource, StringComparison.Ordinal);
        Assert.Contains("/// Scene materialization was preparing to deserialize or had begun deserializing a camera component.\n        CameraComponentDeserialization = 0xC130U", normalizedFailureCodeSource, StringComparison.Ordinal);
        Assert.Contains("/// Scene materialization was preparing to deserialize or had begun deserializing a rounded-rectangle component.\n        RoundedRectComponentDeserialization = 0xC131U", normalizedFailureCodeSource, StringComparison.Ordinal);
        Assert.Contains("/// Scene materialization was preparing to deserialize or had begun deserializing a viewport component.\n        ViewportComponentDeserialization = 0xC132U", normalizedFailureCodeSource, StringComparison.Ordinal);
        Assert.Contains("/// Scene materialization was preparing to deserialize or had begun deserializing a reference-canvas-fit component.\n        ReferenceCanvasFitComponentDeserialization = 0xC133U", normalizedFailureCodeSource, StringComparison.Ordinal);
        Assert.Contains("/// Scene materialization was preparing to deserialize or had begun deserializing the Helen of Code splash component.\n        SplashComponentDeserialization = 0xC134U", normalizedFailureCodeSource, StringComparison.Ordinal);
        Assert.Contains("/// Scene materialization was preparing to deserialize or had begun deserializing a sprite component.\n        SpriteComponentDeserialization = 0xC135U", normalizedFailureCodeSource, StringComparison.Ordinal);
        Assert.Contains("/// The scene manager was about to register textures owned by the newly loaded scene.\n        OwnedTextureRegistration = 0xC140U", normalizedFailureCodeSource, StringComparison.Ordinal);
        Assert.Contains("/// The scene manager was about to register fonts owned by the newly loaded scene.\n        OwnedFontRegistration = 0xC141U", normalizedFailureCodeSource, StringComparison.Ordinal);
        Assert.Contains("/// The scene manager was about to register audio assets owned by the newly loaded scene.\n        OwnedAudioRegistration = 0xC142U", normalizedFailureCodeSource, StringComparison.Ordinal);
        Assert.Contains("/// The scene manager was about to register models owned by the newly loaded scene.\n        OwnedModelRegistration = 0xC143U", normalizedFailureCodeSource, StringComparison.Ordinal);
        Assert.Contains("/// The scene manager was about to register materials owned by the newly loaded scene.\n        OwnedMaterialRegistration = 0xC144U", normalizedFailureCodeSource, StringComparison.Ordinal);
        Assert.Contains("/// The scene manager had registered every owned asset category for the newly loaded scene.\n        OwnedAssetRegistrationCompleted = 0xC145U", normalizedFailureCodeSource, StringComparison.Ordinal);
        Assert.Contains("CoreDraw = 0xC002U", failureCodeSource, StringComparison.Ordinal);
        ...
        int sceneManagerCheckpointEndIndex = normalizedApplicationSource.IndexOf("bool WiiApplication::HasSatisfiedVerificationExitCondition", sceneManagerCheckpointStartIndex, StringComparison.Ordinal);
        Assert.True(drawRefinementStartIndex >= 0 && componentResolverStartIndex > drawRefinementStartIndex);
        Assert.True(sceneManagerCheckpointStartIndex > componentResolverStartIndex && sceneManagerCheckpointEndIndex > sceneManagerCheckpointStartIndex);
        ...
        string sceneManagerCheckpointSource = normalizedApplicationSource.Substring(sceneManagerCheckpointStartIndex, sceneManagerCheckpointEndIndex - sceneManagerCheckpointStartIndex);

        Assert.Contains("if (sceneManagerStage == \"CommitPendingOperationsAtFrameBoundaryBegin\"\n            || sceneManagerStage == \"CommitPendingOperationsAtFrameBoundaryOperation\"\n            || sceneManagerStage == \"CommitPendingOperationsAtFrameBoundaryEnd\") {\n            SetFailureCheckpoint(WiiFailureCode::SceneOperationCommit);", sceneManagerCheckpointSource, StringComparison.Ordinal);
        Assert.Contains("} else if (sceneManagerStage == \"LoadSceneImmediateBegin\"\n            || sceneManagerStage == \"LoadSceneImmediateBeforeResolveSceneContentPath\") {\n            SetFailureCheckpoint(WiiFailureCode::ScenePathResolution);", sceneManagerCheckpointSource, StringComparison.Ordinal);
        Assert.Contains("} else if (sceneManagerStage == \"LoadSceneImmediateAfterResolveSceneContentPath\"\n            || sceneManagerStage == \"LoadSceneImmediateBeforeLoadedSceneRecordLookup\"\n            || sceneManagerStage == \"LoadSceneImmediateAfterLoadedSceneRecordLookup\"\n            || sceneManagerStage == \"LoadSceneImmediateDisposeUntrackedRoots\"\n            || sceneManagerStage == \"LoadSceneImmediateUnloadSingleModeScenes\"\n            || sceneManagerStage == \"LoadSceneImmediateFlushReleasedTextures\"\n            || sceneManagerStage == \"LoadSceneImmediateResetPhysicsTiming\") {\n            SetFailureCheckpoint(WiiFailureCode::SceneRecordLookup);", sceneManagerCheckpointSource, StringComparison.Ordinal);
        Assert.Contains("} else if (sceneManagerStage == \"LoadSceneImmediateBeforeContentLoad\") {\n            SetFailureCheckpoint(WiiFailureCode::SceneContentLoad);", sceneManagerCheckpointSource, StringComparison.Ordinal);
        Assert.Contains("} else if (sceneManagerStage == \"LoadSceneImmediateBeforeSceneLoadServiceLoad\") {\n            SetFailureCheckpoint(WiiFailureCode::SceneMaterializationCall);", sceneManagerCheckpointSource, StringComparison.Ordinal);
        Assert.Contains("} else if (sceneManagerStage == \"LoadSceneImmediateAfterSceneLoadServiceLoad\") {\n            SetFailureCheckpoint(WiiFailureCode::SceneMaterializationReturn);", sceneManagerCheckpointSource, StringComparison.Ordinal);
        Assert.Contains("} else if (sceneManagerStage == \"LoadSceneImmediateBeforeLoadedSceneRecordTrack\") {\n            SetFailureCheckpoint(WiiFailureCode::SceneRecordTrack);", sceneManagerCheckpointSource, StringComparison.Ordinal);
        Assert.Contains("} else if (sceneManagerStage == \"LoadSceneImmediateAfterLoadedSceneRecordListAdd\") {\n            SetFailureCheckpoint(WiiFailureCode::SceneRecordListInsertion);", sceneManagerCheckpointSource, StringComparison.Ordinal);
        Assert.Contains("} else if (sceneManagerStage == \"LoadSceneImmediateAfterLoadedSceneRecordDictionaryAdd\") {\n            SetFailureCheckpoint(WiiFailureCode::SceneRecordDictionaryInsertion);", sceneManagerCheckpointSource, StringComparison.Ordinal);
        Assert.Contains("} else if (sceneManagerStage == \"LoadSceneImmediateBeforeSceneLoadedEvent\") {\n            SetFailureCheckpoint(WiiFailureCode::SceneLoadedEvent);", sceneManagerCheckpointSource, StringComparison.Ordinal);
        Assert.Contains("} else if (sceneManagerStage == \"LoadSceneImmediateAfterSceneLoadedEvent\"\n            || sceneManagerStage == \"LoadSceneImmediateEnd\") {\n            SetFailureCheckpoint(WiiFailureCode::SceneLoadCleanup);", sceneManagerCheckpointSource, StringComparison.Ordinal);
        Assert.Contains("} else if (sceneManagerStage == \"LoadSceneImmediateBeforeRegisterOwnedTextures\") {\n            SetFailureCheckpoint(WiiFailureCode::OwnedTextureRegistration);", sceneManagerCheckpointSource, StringComparison.Ordinal);
        Assert.Contains("} else if (sceneManagerStage == \"LoadSceneImmediateBeforeRegisterOwnedFonts\") {\n            SetFailureCheckpoint(WiiFailureCode::OwnedFontRegistration);", sceneManagerCheckpointSource, StringComparison.Ordinal);
        Assert.Contains("} else if (sceneManagerStage == \"LoadSceneImmediateBeforeRegisterOwnedAudio\") {\n            SetFailureCheckpoint(WiiFailureCode::OwnedAudioRegistration);", sceneManagerCheckpointSource, StringComparison.Ordinal);
        Assert.Contains("} else if (sceneManagerStage == \"LoadSceneImmediateBeforeRegisterOwnedModels\") {\n            SetFailureCheckpoint(WiiFailureCode::OwnedModelRegistration);", sceneManagerCheckpointSource, StringComparison.Ordinal);
        Assert.Contains("} else if (sceneManagerStage == \"LoadSceneImmediateBeforeRegisterOwnedMaterials\") {\n            SetFailureCheckpoint(WiiFailureCode::OwnedMaterialRegistration);", sceneManagerCheckpointSource, StringComparison.Ordinal);
        Assert.Contains("} else if (sceneManagerStage == \"LoadSceneImmediateAfterRegisterOwnedAssets\") {\n            SetFailureCheckpoint(WiiFailureCode::OwnedAssetRegistrationCompleted);", sceneManagerCheckpointSource, StringComparison.Ordinal);
        Assert.Contains("} else {\n            return false;\n        }\n\n        return true;", sceneManagerCheckpointSource, StringComparison.Ordinal);
        int unknownStageFallbackIndex = sceneManagerCheckpointSource.LastIndexOf("} else {", StringComparison.Ordinal);
        string unknownStageFallbackSource = sceneManagerCheckpointSource.Substring(unknownStageFallbackIndex);
        Assert.DoesNotContain("SetFailureCheckpoint", unknownStageFallbackSource, StringComparison.Ordinal);

        Assert.Contains("if (sceneTransitionStage == \"AfterSceneLoadedEventDispatch\") {\n            SetFailureCheckpoint(WiiFailureCode::SceneLoadedEventReturned);", drawRefinementSource, StringComparison.Ordinal);
        Assert.Contains("} else if (sceneTransitionStage == \"BeforeSceneLoadedEventArgsRelease\") {\n            SetFailureCheckpoint(WiiFailureCode::SceneLoadedEventArgsRelease);", drawRefinementSource, StringComparison.Ordinal);
        Assert.Contains("} else if (sceneTransitionStage == \"AfterSceneLoadedEventArgsRelease\") {\n            SetFailureCheckpoint(WiiFailureCode::SceneLoadedEventArgsReleased);", drawRefinementSource, StringComparison.Ordinal);
        Assert.Contains("} else if (sceneTransitionStage == \"AfterTransitionSceneAssetRelease\"\n            || sceneTransitionStage == \"AfterSceneTransitionCommit\") {\n            SetFailureCheckpoint(WiiFailureCode::SceneLoadCleanup);", drawRefinementSource, StringComparison.Ordinal);

        int nativeStageIndex = drawRefinementSource.IndexOf("GetLastDrawStage()", StringComparison.Ordinal);
        ...
        int outerGeneratedMappingIndex = drawRefinementSource.IndexOf("sceneTransitionStage == \"BeforeCompleteFrameBoundary\"", StringComparison.Ordinal);
        int outerDrawBeginIndex = drawRefinementSource.IndexOf("sceneTransitionStage == \"DrawBegin\"", StringComparison.Ordinal);
        Assert.True(nativeStageIndex >= 0 && directMarkerIndex > nativeStageIndex);
        Assert.True(sceneLoadOwnershipIndex > directMarkerIndex && sceneManagerOwnershipIndex > sceneLoadOwnershipIndex);
        Assert.True(outerGeneratedMappingIndex > sceneManagerOwnershipIndex && outerDrawBeginIndex > outerGeneratedMappingIndex);

        string sceneManagerOwnershipSource = drawRefinementSource.Substring(sceneManagerOwnershipIndex, outerGeneratedMappingIndex - sceneManagerOwnershipIndex);
        Assert.Contains("sceneTransitionStage.starts_with(\"SceneManager:\")\n            && EngineCore != nullptr\n            && EngineCore->get_SceneManager() != nullptr\n            && RefineSceneManagerFailureCheckpoint(EngineCore->get_SceneManager()->get_LastTraceStage())", sceneManagerOwnershipSource, StringComparison.Ordinal);

        int coreDrawCheckpointIndex = normalizedApplicationSource.IndexOf("SetFailureCheckpoint(WiiFailureCode::CoreDraw);", StringComparison.Ordinal);
        int generatedDrawCallIndex = normalizedApplicationSource.IndexOf("EngineCore->Draw();", coreDrawCheckpointIndex, StringComparison.Ordinal);
        Assert.True(coreDrawCheckpointIndex >= 0 && generatedDrawCallIndex > coreDrawCheckpointIndex);
    }
